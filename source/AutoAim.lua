...
RunService.RenderStepped:Connect(function()
	...
	if not lockedTarget or not lockedTarget:IsDescendantOf(Workspace) or lockedTarget.Parent:FindFirstChildOfClass("Humanoid").Health <= 0 then
		lockedTarget = getNearestEnemyHead()
	end

	if lockedTarget then
		local predicted = lockedTarget.Position + lockedTarget.Velocity * 0.04
		local camPos = camera.CFrame.Position
		local desiredCFrame = CFrame.new(camPos, predicted)
		camera.CFrame = camera.CFrame:Lerp(desiredCFrame, SMOOTH_FACTOR)

		local viewport = camera:WorldToViewportPoint(lockedTarget.Position)
